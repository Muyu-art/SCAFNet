...
def draw_image_panel(ax, image, gt_boxes=None, pred_boxes=None):
    ax.imshow(image)
    ax.axis("off")
...
def save_single_image_panel(save_file, image, gt_boxes=None, pred_boxes=None):
    h, w = image.shape[:2]
    fig = plt.figure(figsize=(w / 100.0, h / 100.0), frameon=False)
    ax = plt.Axes(fig, [0., 0., 1., 1.])
    fig.add_axes(ax)
    draw_image_panel(ax, image=image, gt_boxes=gt_boxes, pred_boxes=pred_boxes)
...
def save_single_bev_panel(
    ...
    fig = plt.figure(figsize=figsize, facecolor="black", frameon=False)
    ax = plt.Axes(fig, [0., 0., 1., 1.])
    fig.add_axes(ax)
    ...
    plt.savefig(save_file, dpi=220, bbox_inches="tight", pad_inches=0, facecolor="black")
    plt.close(fig)
...
def main():
    ...
    parser.add_argument("--x_max", type=float, default=70.4)
    parser.add_argument("--y_min", type=float, default=-40.0)
    parser.add_argument("--y_max", type=float, default=40.0)
    parser.add_argument("--point_size", type=float, default=0.15)
    parser.add_argument("--point_color", type=str, default="white")
    parser.add_argument(
        "--output_dir",
        type=str,
        default="output/panel30",
        help="Root output dir; image/ and bev/ will be created under it"
    )
    ...
    bev_out_dir = os.path.join(output_dir, "bev")
    ...
    panel_defs = [
        ...
        ("CasA-V", "CasA-V"),
        ("SCAFNet", "SCAFNet"),
        ("CasA_V_fgsp_V1", "CasA_V_fgsp_V1"),
        ("SCAF-FGSPNet", "SCAF-FGSPNet"),
    ]
    ...
    for frame_id in frame_ids:
        ...
        for panel_name, model_name in panel_defs:
            ...
            safe_panel_name = sanitize_name(panel_name)

            image_save_path = os.path.join(
                image_out_dir, f"{frame_id}_{safe_panel_name}.png"
            )
            ...
            print(f"[SAVE][image] {image_save_path}")
            saved_count += 1

            bev_save_path = os.path.join(
                bev_out_dir, f"{frame_id}_{safe_panel_name}.png"
            )
            ...
            print(f"[SAVE][bev] {bev_save_path}")
            saved_count += 1

    print("=" * 100)
    print(f"Saved panels to: {output_dir}")
    print("=" * 100)
    print(f"image panels: {image_out_dir}")
    print(f"bev panels  : {bev_out_dir}")
